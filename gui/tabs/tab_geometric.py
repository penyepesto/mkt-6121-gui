...
from ..ops.geometric_ops import get_operations as get_geo_ops


class GeometricTab(QWidget):
    """‘Geometric Methods’ sekmesi – feature detection / transform / advanced."""
    def __init__(self, input_manager, canvas, parent=None):
        super().__init__(parent)
        self.input  = input_manager
        self.canvas = canvas
        self.ops    = get_geo_ops()

        self.toggle = TogglePanel(self.ops)
        self.params = ParamPanel(self.ops)

        self.btn_run = QPushButton("Run Geometric Ops")
